...
data$Bipolar <- ifelse(data$Diagnosis == 3,1,0)
...
title <- textGrob("Feature Importance (Bipolar Disorder)", gp = gpar(fontsize = 20, fontface = "bold", fontfamily="Times New Roman"))
...
remaining <- unique(results[results$ACC == 1, "Subject"]) # 28 of 54
...
top_bipolar <- top_feature_groups %>% filter(Prognosis == 1) %>% arrange(desc(Count)) %>% head(15) %>% select(Feature1, Feature2, Feature3, Feature4, Feature5)
top_features <- rbind(top_normal, top_bipolar) %>% unique()
...
normal_score <- (true_count / length(unique(normal_score$Subject))) # 89

bipolar_score <- scored_results[scored_results$Condition == "1",]
true_count <- bipolar_score %>% group_by(Subject) %>% filter(Count == max(Count)) %>% filter(Condition == Test) %>% nrow()
bipolar_score <- (true_count / length(unique(bipolar_score$Subject))) # 67

bipolar_features <- results %>% filter(Condition == "1" & Condition == Test) %>% 
  group_by(Feature1, Feature2, Feature3, Feature4, Feature5) %>% summarise(Count = n()) %>% 
  arrange(desc(Count)) %>% head(5)
bipolar_features <- c(bipolar_features$Feature1, bipolar_features$Feature2, 
                      bipolar_features$Feature3, bipolar_features$Feature4, bipolar_features$Feature5)
bipolar_features <- as.data.frame(bipolar_features) %>% group_by(bipolar_features) %>% summarise(Rank=n()) %>%
  arrange(desc(Rank)) %>% head(5)
names(bipolar_features) <- c("Feature", "Rank")

ggplot(data=bipolar_features, aes(x=reorder(Feature, -Rank), y=Rank)) +
  geom_bar(stat="identity", fill="steelblue2") + 
  ...
  ggtitle("Bipolar Feature Importance") +
