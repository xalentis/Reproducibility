...
set.seed(42)
temp <- data
a <- createDataPartition(temp$Type, p = 0.8, list=FALSE)
...
for (trial in 1:nrow(data))
{
  ...
  acc <- (sum(yhat == test$Type) / nrow(test))
...
test <- temp[-a,]
start_time <- Sys.time()
rf <- train(Type ~ ., data=train, importance=TRUE, method="rf", trControl=repeat_cv, metrics="Accuracy")
runtime <- Sys.time() - start_time
print(runtime)
rf <- rf$finalModel
varimp <- importance(rf)
...
ggplot(data=varimp, aes(x=reorder(Features,MSE), y=MSE)) +
  ...
  theme_classic() + 
  theme(text = element_text(size = 20), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle("Feature Importance - Breast Cancer - CV + Seed(42)") +
  xlab("Feature") +
  theme(plot.title = element_text(hjust = 0.5))
