...
set.seed(42)
temp <- mtcars
a <- createDataPartition(temp$cyl, p = 0.8, list=FALSE)
temp$cyl <- as.factor(temp$cyl)
temp$vs <- as.factor(temp$vs)
temp$am <- as.factor(temp$am)
temp$gear <- as.factor(temp$gear)
temp$carb <- as.factor(temp$carb)
train <- temp[a,]
test <- temp[-a,]
rf <- randomForest(cyl ~ ., data=train, importance=TRUE)
...
ggplot(data=varimp, aes(x=reorder(Features,MSE), y=MSE)) +
  ...
  ggtitle("Feature Importance - Cars - Seed(42)") +
  xlab("Feature") +
...
ggplot(data=varimp, aes(x=reorder(Features,MSE), y=MSE)) +
  ...
  theme_classic() + 
  theme(text = element_text(size = 20), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle("Feature Importance - Cars - Seed(43)") +
  xlab("Feature") +
  theme(plot.title = element_text(hjust = 0.5))
...
for (trial in 1:nrow(mtcars))
{
  ...
  acc <- (sum(yhat == test$cyl) / nrow(test))
...
test <- temp[-a,]
start_time <- Sys.time()
rf <- train(cyl ~ ., data=train, importance=TRUE, method="rf", trControl=repeat_cv, metrics="Accuracy")
runtime <- Sys.time() - start_time
print(runtime)
rf <- rf$finalModel
varimp <- importance(rf)
