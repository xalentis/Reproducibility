...
accuracy <- sum(results$ACC=="1") / nrow(results) # 97.51
...
min_trial <- results$Trial # minimum number of trials required to provide stable feature importance = 256

# Bare.nuclei, Cell.size, Cell.shape, Cl.thickness, Normal.nucleoli = 0.9705882
importance <- importance %>% na.omit() %>% group_by(Feature) %>% summarise(Rank=sum(Rank)) %>% arrange(desc(Rank))
plot6 <- ggplot(data=importance, aes(x=reorder(Feature, -Rank), y=Rank)) +
  geom_bar(stat="identity", fill="dodgerblue2") +
  theme_classic() + 
  ggtitle("256 Random Trials") +
  xlab("Feature") +
  theme(
    axis.title = element_text(size = 14, family="Times New Roman", face="bold"),
    axis.text = element_text(size = 11, family = "Times New Roman", face="bold"),
...
title <- textGrob("Feature Importance (Breast Cancer)", gp = gpar(fontsize = 20, fontface = "bold", fontfamily="Times New Roman"))
